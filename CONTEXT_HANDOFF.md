...
- v2.4.46 后继续放宽锚点采集底部接受条件：`acceptBottom` 贴近 ListView 底部并受屏幕 94% 保护；已看到候选消息后允许边界短暂丢失最多 3 次，避免第一条消息已可见但被 `TOO_LOW` 拒绝。
- v2.4.47 后拆分“可识别”和“可长按”：底部只露出一点的第一条消息不会直接作为 SAFE 返回；会继续按候选消息复定位并上移到可长按区，避免 y=194x 长按不出多选。
